# BA-Agent 项目记忆

> 本文件是 BA-Agent 的项目级记忆（Layer 3: Context Bootstrap）
> 团队共享知识，包含在 Git 版本控制中

## 项目概述

BA-Agent (Business Analysis Agent) 是一个商业分析助手 Agent，面向非技术业务人员。

**目标用户**：跨境电商企业的业务分析师、运营人员
**核心价值**：通过自然语言对话完成数据异动检测、归因分析、报告生成、数据可视化

## 技术架构

```
┌─────────────────────────────────────────────────────────────┐
│                      BA-Agent 架构                          │
├─────────────────────────────────────────────────────────────┤
│  Frontend: IM Bot (企业微信/钉钉) + Excel 插件              │
├─────────────────────────────────────────────────────────────┤
│  API Layer: FastAPI (Python)                                │
├─────────────────────────────────────────────────────────────┤
│  Agent Layer: LangGraph Agent + Claude 3.5 Sonnet           │
│  ├─ System Prompt (AGENTS.md)                               │
│  ├─ Memory Management (三层记忆)                            │
│  └─ Tool Invocation (langchain.agents.create_agent)         │
├─────────────────────────────────────────────────────────────┤
│  Tools Layer (基础工具)                                      │
│  ├─ execute_command (Docker隔离)                            │
│  ├─ run_python (Docker隔离，核心)                           │
│  ├─ web_search (MCP: mcp__web-search-prime__webSearchPrime) │
│  ├─ read_webpage (MCP: mcp__web_reader__webReader)          │
│  ├─ read_file (CSV/Excel/JSON/文本/Python/SQL)              │
│  ├─ query_database (SQLAlchemy)                             │
│  ├─ search_knowledge (Chroma向量检索)                       │
│  └─ invoke_skill (调用可配置Skills)                         │
├─────────────────────────────────────────────────────────────┤
│  Skills Layer (可配置分析Skills)                            │
│  ├─ anomaly_detection (异动检测)                            │
│  ├─ attribution (归因分析)                                  │
│  ├─ report_gen (报告生成)                                   │
│  └─ visualization (数据可视化，ECharts)                      │
├─────────────────────────────────────────────────────────────┤
│  Data Layer                                                  │
│  ├─ PostgreSQL (主数据库)                                   │
│  ├─ ClickHouse (大数据分析)                                 │
│  └─ Chroma (向量数据库，知识检索)                           │
└─────────────────────────────────────────────────────────────┘
```

## 核心技术栈

| 组件 | 技术 | 版本 |
|------|------|------|
| LLM | Claude (Anthropic) | 3.5 Sonnet |
| Agent 框架 | LangGraph | >=1.0.7 |
| 工具框架 | LangChain Core | >=1.2.0 |
| API 框架 | FastAPI | >=0.115.0 |
| 数据分析 | pandas, numpy, scipy | 最新 |
| 科学统计 | statsmodels, scikit-learn | 最新 |
| Excel 处理 | openpyxl, xlrd, xlsxwriter | 最新 |
| 向量数据库 | Chroma | (Python 3.14 ARM64 暂不支持) |

## 重要设计决策

### API 选择 (2025-02-04)
- **Agent 创建**: 使用 `langchain.agents.create_agent` (LangChain 1.2.8+ 推荐方式)
- **工具定义**: 使用 `langchain_core.tools.StructuredTool`
- **记忆管理**: 使用 `langgraph.checkpoint.memory.MemorySaver` 支持对话历史

### MCP 工具集成 (2025-02-04)
- **Web 搜索**: 使用 `mcp__web-search-prime__webSearchPrime` (Z.ai MCP)
- **Web 读取**: 使用 `mcp__web_reader__webReader` (Z.ai MCP)

### 数据可视化方案 (2025-02-04)
- **方案**: ECharts (通过 Gemini 3 Pro 生成代码，前端渲染)
- **原因**: 交互性强、中文支持好、无需 Python 图表库

### Python 版本限制 (2025-02-04)
- **当前**: Python 3.12 ARM64
- **已知问题**: Python 3.14 ARM64 暂不支持 chromadb (缺少 onnxruntime)

### 统一工具输出格式系统 (2025-02-05)
- **目的**: 统一所有工具的输出格式，支持模型间上下文传递和工程遥测
- **核心文件**: `models/tool_output.py`, `tools/base.py`
- **功能特性**:
  - **响应格式**: CONCISE/STANDARD/DETAILED/RAW 四种模式
  - **模型上下文**: summary (摘要), observation (ReAct 格式), result (详细数据)
  - **工程遥测**: ToolTelemetry 收集延迟、Token 使用、错误追踪、缓存状态
  - **ReAct 兼容**: 标准化 Observation 格式，兼容 LangChain/LangGraph
  - **Token 优化**: 紧凑格式、YAML、XML 等多种输出格式
- **使用方式**: 使用 `@unified_tool` 装饰器包装现有工具函数
- **参考**: 基于 Anthropic、Claude Code、Manus 等 Agent 产品的最佳实践

## 三层记忆管理

每次会话开始时，Agent 按以下顺序加载记忆：

1. **读取 `AGENTS.md`** - Agent 系统指令和记忆指南
2. **读取 `USER.md`** - 用户信息和偏好
3. **读取 `MEMORY.md`** - 长期策划知识
4. **读取 `memory/YYYY-MM-DD.md`** (今天和昨天) - 最近上下文

### 记忆写入规则

| 触发条件 | 目标位置 | 示例 |
|----------|----------|------|
| 日常笔记、"记住这个" | `memory/YYYY-MM-DD.md` | "讨论了 API 设计" |
| 持久事实、偏好、决策 | `MEMORY.md` | "用户偏好 TypeScript" |
| 重要里程碑 | `CLAUDE.md` | "完成 API 重构" |

## 记忆管理工具

Agent 可以使用以下工具管理记忆：

- **`memory_search`**: 语义搜索 MEMORY.md + memory/*.md
- **`memory_get`**: 读取特定内存文件的特定行
- **`memory_write`**: 写入记忆（自动选择 Layer 1 或 Layer 2）

## 开发规范

### 代码风格
- 所有函数必须有类型注解
- 复杂逻辑必须有文档字符串
- 所有外部调用必须有错误处理

### 安全要求
- Docker 容器必须有资源限制
- 命令行和 Python 执行必须有白名单
- SQL 查询必须参数化防止注入

### 测试要求
- 每个工具必须有单元测试
- 每个 Skill 必须有单元测试
- Agent 需要端到端测试
- 测试覆盖率 > 80%

## 当前进度

查看 `scripts/ralph/progress.txt` 获取详细进度。

**已完成**:
- ✅ US-001: 项目初始化
- ✅ 依赖安装和 LangChain 1.2.x API 修复

**进行中**:
- 🔄 US-002: 核心数据模型定义 (Pydantic)

**待开始**:
- ⏳ US-003: 配置管理系统
- ⏳ US-004: LangGraph Agent 基础框架
- ... (详见 prd.json)

## 参考资料

- [LangChain 1.2.x 文档](https://python.langchain.com/docs/tutorials/agents/)
- [Claude API 文档](https://docs.anthropic.com/)
- [Manus AI - Planning with Files](https://github.com/OthmanAdi/planning-with-files)
- [Clawdbot Memory Architecture](https://manthanguptaa.in/posts/clawdbot_memory/)
