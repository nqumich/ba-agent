...
import { Send, Paperclip, Mic, ArrowLeft, Copy, RotateCw, ThumbsUp, ThumbsDown } from "lucide-react";
...
export function ChatInterface({ initialMessages, onUpdateMessages, onBack }) {
  ...
  // 发送消息到后端并更新会话（/api 由 Vite 代理到 localhost:8000）
  const sendToBackend = async (text) => {
    try {
      ...
      const hint = isNetwork || err.message.includes('500') || err.message.includes('502')
        ...
        : '';
      const errContent = `请求失败：${err.message}${hint ? ' ' + hint : ''}`;
      setMessages(prev => {
        const next = [...prev, { role: 'ai', content: errContent }];
        onUpdateMessages?.(next);
        return next;
      });
    } finally {
  ...
  // 进入对话时若只有一条用户消息且未请求过，则请求首条回复（仅执行一次）
  useEffect(() => {
          ...
          {messages.map((msg, index) => (
            ...
            >
               ...
               <div className={`flex items-center gap-1 mt-1 ${msg.role === 'user' ? 'mr-1' : 'ml-12'}`}>
                  <Button variant="ghost" size="icon" className="h-6 w-6 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded">
