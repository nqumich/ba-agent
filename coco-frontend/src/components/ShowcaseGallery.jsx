...
export function ShowcaseGallery({ onClose }) {
  ...
  // 将静态 mockData 转为 State
  const [galleryData, setGalleryData] = useState(initialMockData);

  // 处理新案例发布
  const handlePublishCase = (newCaseData) => {
    // 根据分类匹配图标和关键词
    let icon = <Sparkles className="h-3.5 w-3.5" />;
    let imageKeyword = "technology,abstract,minimalist";
    let tag = "Case";

    switch(newCaseData.category) {
        case 'excel':
            icon = <FileSpreadsheet className="h-3.5 w-3.5" />;
            imageKeyword = "excel,data,spreadsheet,minimalist";
            tag = "Data";
            break;
        case 'chart':
            icon = <PieChart className="h-3.5 w-3.5" />;
            imageKeyword = "chart,graph,analytics,clean";
            tag = "Viz";
            break;
        case 'report':
            icon = <FilePenLine className="h-3.5 w-3.5" />;
            imageKeyword = "document,writing,paper,workspace";
            tag = "Doc";
            break;
        default:
            break;
    ...
    const newCaseItem = {
        ...
        tag: newCaseData.tags || tag,
        imageKeyword: imageKeyword,
        icon: icon,
        desc: newCaseData.desc // 虽然 GalleryCard 目前没显示描述，但数据可以存着
    };

    setGalleryData(prev => ({
        ...prev,
        square: [newCaseItem, ...prev.square] // 添加到案例广场的最前面
    }));

    // 自动切换到案例广场并重置排序，以便用户看到新发布的案例
    setActiveTab('square');
    setSortOrder('latest');
  };
  ...
  // 过滤与排序逻辑
  const items = useMemo(() => {
    let currentItems = galleryData[activeTab] || [];
    ...
    return currentItems;
  }, [activeTab, searchQuery, sortOrder, galleryData]);
        ...
        <div className="max-w-7xl mx-auto p-6 min-h-full">
             ...
             <div className="flex flex-col gap-4 mb-8">
                <div className="flex items-center justify-between gap-4">
                    ...
                    {/* 仅在案例广场显示上传按钮 */}
                    {activeTab === 'square' && (
                        <UploadCaseDialog onPublish={handlePublishCase}>
