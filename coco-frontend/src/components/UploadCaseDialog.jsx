...
import { Image as ImageIcon, CloudUpload, Loader2, X, Upload, FileText } from 'lucide-react';
import React, { useRef, useState } from 'react';
import { DialogContent, DialogTitle, DialogFooter, DialogClose, Dialog, DialogHeader, DialogTrigger } from '@/components/ui/dialog';
import { Label } from '@/components/ui/label';
import { Button } from '@/components/ui/button';
export function UploadCaseDialog({ children, onPublish }) {
  ...
  const handleSubmit = async () => {
      ...
      try {
          ...
          // 调用父组件传入的回调函数，真正的添加数据
          if (onPublish) {
              onPublish(formData);
          }

          toast.success("案例发布成功", {
              description: "您的案例已发布至案例广场最新列表。",
          });
           ...
           <div className="space-y-2">
             ...
             >
                <input 
                    ...
                    multiple 
                    accept=".xlsx,.xls,.pdf,.doc,.docx,.ppt,.pptx,.txt,.csv,.png,.jpg,.jpeg"
                />
                <div className="w-12 h-12 bg-blue-50 text-blue-600 rounded-full flex items-center justify-center mb-3">
                    <CloudUpload className="h-6 w-6" />
                </div>
                <p className="text-sm font-medium text-gray-900">点击或拖拽文件到此处</p>
                <p className="text-xs text-gray-500 mt-1">支持 Excel, PDF, Word, PPT, 图片等 (最大 50MB)</p>
             </div>
