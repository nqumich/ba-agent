...
2. **All functionality in `ToolExecutionResult`**: Single class handles all tool results
3. **Renamed `request_id` → `tool_call_id`**: Consistent naming with LangChain
4. **Added `from_raw_data()` factory method**: For creating results from raw data
...
# NOTE: ToolResultMessage has been REMOVED (v2.0.0)
# All tool result functionality is now consolidated in ToolExecutionResult
# See Tool ↔ Agent Communication Protocol section below for the unified model.
...
class ToolExecutionResult(BaseModel):
    ...
    def from_raw_data(
        ...
        """
        Create ToolExecutionResult from raw data with specified output level.
...
Tool Call ID: {self.tool_call_id}"""
    ...
    def to_tool_message(self):
        ...
        Example:
            result = ToolExecutionResult.from_raw_data(tool_call_id, data, OutputLevel.STANDARD)
...
class ToolTimeoutHandler:
    ...
    def execute_with_timeout(
        ...
        Args:
            ...
            tool_call_id: Tool call ID from AIMessage.tool_calls[i]["id"]
