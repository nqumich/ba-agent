...
// 本地开发时若 SDK 未就绪，超时后仍渲染页面，避免一直空白
const DEV_FALLBACK_MS = 3000;
...
export const NoCodeProvider = ({ children }) => {
  ...
  // 开发环境：SDK 长时间未就绪时仍显示页面，方便本地调试
  useEffect(() => {
    if (!import.meta.env.DEV || isReady || devFallback) return;
    const t = setTimeout(() => setDevFallback(true), DEV_FALLBACK_MS);
  ...
  const shouldRender = isReady || (import.meta.env.DEV && devFallback);
